...
class NameEingabeFenster(Nebenfenster):
	...
	def interface_generieren(self, spielerzahl: int = None):
		...
		standard_entry = Entry(self.entry_frame)
		standard_entry.insert(0, config["Spiel"]["nutzername"])
		standard_entry.pack(fill=X)
		self.entries.append(standard_entry)
	...
	def eingabe(self):
		...
		for entry in self.entries:
			if name := entry.get():
				if name in namen:
					messagebox.showerror("Doppelter Name", "Bitte geben sie für jeden Spieler einen EINZIGARTIGEN Namen ein.")
					return

				namen.append(name)
			else:
				messagebox.showerror("Ungültiger Name", "Bitte tragen Sie für jeden Spieler einen GÜLTIGEN Namen ein.")
				return
